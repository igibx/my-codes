// ========================================================================================
// SUPERGEMINI CREATOR ENGINE - V23.0 (THE DYNAMIC SHADOWS & FOREST UPDATE)
// Architect: Gemini God Mode | Executive Producer: Igor Bettarello Xavier (OMEGARED)
// Features: Dynamic 1:1 Shadows, Specular Shaders, Full Anim Tree (Idle, Walk, Run, Jump), Forest Pack
// ========================================================================================
...
// ==============================================================================
// 1. SISTEMAS CORE E SHADERS GLSL AVANÇADOS (SPECULAR E RIM LIGHT)
// ==============================================================================
...
void main() {
    ...
    // Iluminação difusa básica
    float diff = max(dot(norm, light), 0.3);
    
    // Iluminação Rim (Luz de borda baseada na posição da câmera)
    vec3 viewDir = normalize(viewPos - fragPosition);
    float rim = 1.0 - max(dot(viewDir, norm), 0.0);
    rim = smoothstep(0.6, 1.0, rim);
    
    // Reflexo Especular (Para o corpo do Zuba brilhar)
    vec3 reflectDir = reflect(-light, norm);
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), 16.0);
    
    vec3 lighting = vec3(diff) + vec3(rim * 0.4) + vec3(spec * 0.3);
    
    // Garante que a sombra não fique 100% preta
...
public:
    ...
    Model modelo3D; Texture2D texturaAvatar; Shader shaderLuz; int lightDirLoc; int viewPosLoc;
    ...
    CompanheiroMagico() : posicao({0, 15, 0}), alvoMovimento({0,0,0}), alvoLaser({0,0,0}), nome("Zuba"), tempoOrbita(0.0f), estadoFisico(ORBITAR), 
                          ...
                          animacoes(nullptr), animCount(0), animFrameCounter(0.0f), ultimoAnimIndex(0), temModelo(false), temTextura(false), anguloRotacao(0.0f) {}
    ...
    void CarregarModeloAvatar(const char* pathModelo, const char* pathTextura) {
        if (FileExists(pathModelo)) {
            ...
            viewPosLoc = GetShaderLocation(shaderLuz, "viewPos");
    ...
    void Iniciar() {
        ...
        AdicionarMensagem(nome, Config.Tr("Sistemas V23 Prontos! Sombra projetada e animacoes arrumadas!", "V23 Systems Ready! Projected shadows and fixed animations!"), GOLD);
    }
    ...
    void AtualizarFisica(Vector3 posJogador, Vector3 forwardJogador, float dt) {
        ...
        float anguloAlvo = anguloRotacao; // Usado para rotacionar o zuba suavemente!
        ...
        // Interpolação suave de rotação (Zuba faz curvas suaves ao invés de viradas bruscas)
        ...
        // FIX DE ALTURA: Se tem modelo, Zuba não tem Offset em Y e os pés ficam colados no chão!
        float zubaYOffset = temModelo ? 0.0f : (0.2f * escalaAtual); 
        posicao.y = Lerp(posicao.y, chaoAura + zubaYOffset + puloY, trueDelta * 18.0f); 

        // NOVO MOTOR DE ANIMAÇÕES PERFEITO (IDLE, WALK, RUN, JUMP)
        if (temModelo && animCount > 0) {
            int currentAnimIndex = 0;
            if (zubaPuloTimer > 0) {
                currentAnimIndex = (animCount > 3) ? 3 : 0; 
            } else if (isMoving) {
                if (velZuba > 15.0f) currentAnimIndex = (animCount > 2) ? 2 : ((animCount > 1) ? 1 : 0);
                else currentAnimIndex = (animCount > 1) ? 1 : 0;
            } else if (zubaDancando) {
                currentAnimIndex = (animCount > 2) ? 2 : 0; 
            } else {
                currentAnimIndex = 0; // IDLE
            }

            if (currentAnimIndex != ultimoAnimIndex) {
                animFrameCounter = 0;
                ultimoAnimIndex = currentAnimIndex;
            }

            // FIX DE VELOCIDADE: As perninhas giram exatamente de acordo com a velocidade dele de deslocamento!
            ...
            // Avança o frame suavemente para qualquer estado, incluindo IDLE!
    ...
    void Desenhar3D(Vector3 cameraPos) {
        float r = 0.6f * escalaAtual; 
        
        if (temModelo) {
            // Passa a posição da câmera pro shader do Zuba calcular o brilho Specular / Rim Light
            float cpos[3] = { cameraPos.x, cameraPos.y, cameraPos.z };
            SetShaderValue(shaderLuz, viewPosLoc, cpos, SHADER_UNIFORM_VEC3);
        }
        
        // SOMBRA 1:1 REAL-TIME (Projeta a geometria exata achatada no chão)
        if (!camuflagemAtiva && formaAtual == "ESFERA" && temModelo) {
            ...
            // O Shader V23 já lida perfeitamente com tint preto. Não precisamos mais "zerar" o shader do material (isso causava o crash!).
...
void DrawCreditsMenu(int larguraTela, int alturaTela, Font font, GameState& estadoAtual, float dt) {
    ...
    DrawTextCenter("V23.0 - THE DYNAMIC SHADOWS & FOREST UPDATE", startY + 60, 20, LIGHTGRAY);
...
// ==============================================================================
int main(void) {
    InitWindow(1920, 1080, "SUPERGEMINI CREATOR ENGINE - V23.0 THE DYNAMIC SHADOWS UPDATE");
    ToggleFullscreen(); 
    SetExitKey(0); SetTargetFPS(60);
    Audio.Init(); 
    Forest.Init(); // Carrega o Forest Pack Real!
    ...
    while (!WindowShouldClose() && !sairDoJogo) {
        ...
        if (estadoAtual == MENU) {
            ...
            DrawTextCenter("V23.0 THE DYNAMIC SHADOWS & FOREST UPDATE", alturaTela/2 - 180, 20, LIGHTGRAY);
            ...
            BeginDrawing();
                ...
                BeginMode3D(renderCam);
                    ...
                    // RENDERIZAÇÃO DA FLORESTA REAL (FOREST PACK GLTF)
                    if (!modoWireframe && biomaAtual != 4) {
                        // FIX: Amarração na malha de Mundo Absoluta (Elimina as Árvores Piscando)
                        int startX = ((int)floor(camera.position.x) - rDistFinal) & ~1;
                        int startZ = ((int)floor(camera.position.z) - rDistFinal) & ~1;
                        int endX = ((int)floor(camera.position.x) + rDistFinal) & ~1;
                        int endZ = ((int)floor(camera.position.z) + rDistFinal) & ~1;

                        for (int wX = startX; wX <= endX; wX+=2) {
                            for (int wZ = startZ; wZ <= endZ; wZ+=2) {
                                float distCentro = sqrt(pow(wX - camera.position.x, 2) + pow(wZ - camera.position.z, 2));
                                if (distCentro > rDistFinal) continue;
                                ...
                                if (biomaAtual == 0 && biomaAlvo == 0 && slope < 0.3f && h >= 0.0f && h < 15.0f && pathNoise > 0.05f) {
                                    float prob = RandPseudo(wX, wZ);
                                    Color tint = ApplyFog(WHITE, distCentro);

                                    if (prob > 0.98f && !Forest.trees.empty()) {
                                        int idx = (int)(prob * 10000) % Forest.trees.size();
                                        DrawModelEx(Forest.trees[idx], {(float)wX, h, (float)wZ}, {0,1,0}, prob*360.0f, {1.0f, 1.0f, 1.0f}, tint);
                                    } 
                                    else if (prob > 0.94f && !Forest.bushes.empty()) {
                                        int idx = (int)(prob * 10000) % Forest.bushes.size();
                                        DrawModelEx(Forest.bushes[idx], {(float)wX, h, (float)wZ}, {0,1,0}, prob*360.0f, {1.0f, 1.0f, 1.0f}, tint);
                                    } 
                                    else if (prob > 0.88f && !Forest.rocks.empty()) {
                                        int idx = (int)(prob * 10000) % Forest.rocks.size();
                                        DrawModelEx(Forest.rocks[idx], {(float)wX, h, (float)wZ}, {0,1,0}, prob*360.0f, {1.0f, 1.0f, 1.0f}, tint);
                                    } 
                                    else if (prob > 0.70f && !Forest.grasses.empty()) {
                                        int idx = (int)(prob * 10000) % Forest.grasses.size();
                                        DrawModelEx(Forest.grasses[idx], {(float)wX, h, (float)wZ}, {0,1,0}, prob*360.0f, {1.0f, 1.0f, 1.0f}, tint);
                                    }
                                }
                            }
                        }
                    ...
                    if (!modoWireframe && (biomaAtual != 1 && biomaAlvo != 1) && (biomaAtual != 3 && biomaAlvo != 3)) {
                        rlBegin(RL_QUADS);
                        // FIX: Amarra também a Água na malha Absoluta (A água fica paradinha, sem jitter ao se mover)
                        int startX = ((int)floor(camera.position.x) - rDistFinal) & ~1;
                        int startZ = ((int)floor(camera.position.z) - rDistFinal) & ~1;
                        int endX = ((int)floor(camera.position.x) + rDistFinal) & ~1;
                        int endZ = ((int)floor(camera.position.z) + rDistFinal) & ~1;
                    ...
                    companheiro.Desenhar3D(camera.position); // Passamos a camera pro Zuba para cálculos de Reflexo!
                EndMode3D();
    ...
    Forest.Unload(); // Limpa as texturas e modelos da Floresta real!
    Audio.Close();
