// ========================================================================================
// SUPERGEMINI CREATOR ENGINE - V22.0 (THE OMNISCIENCE EXPANSION)
...
// New in V22: Chat History, T-Bug Fixed, Smaller UI, Massive Content Update!
// ========================================================================================
...
// ==============================================================================
// 1. SISTEMAS CORE E SHADERS GLSL INJETADOS
// ==============================================================================
...
Color ObterCorBiomaPuro(int bioma, float x, float z, float altura, Vector3 normal) {
    ...
    else if (bioma == 4) { if (slope > 0.3f) return { 10, 10, 25, 255 }; if (pathNoise < 0.02f) return {0, 255, 255, 255}; return { 30, 0, 50, 255 }; } // Cyberpunk
    else if (bioma == 5) { if (slope > 0.5f) return { 50, 0, 100, 255 }; return { 80, 255, 100, 255 }; } // Alien
    else if (bioma == 6) { if (slope > 0.5f) return { 255, 105, 180, 255 }; return { 255, 182, 193, 255 }; } // Doce
...
RespostaIA ProcessarIAInterna(std::string inputJogador, float alturaAtual) {
    ...
    if (Match(in, {"god mode", "deus"})) { cmds["HACK"] = isOff ? "GODMODE_OFF" : "GODMODE_ON"; resp += Config.Tr("Poder absoluto (F voa, C desce). ", "Absolute power. "); acted = true; }
...
public:
    ...
    Model modelo3D; Texture2D texturaAvatar; Shader shaderLuz; int lightDirLoc;
    ModelAnimation* animacoes; int animCount; int animFrame;
    bool temModelo; bool temTextura; float anguloRotacao;

    // Ações Físicas Extras
    float zubaPuloTimer = 0.0f;
    float zubaGiroVelocidade = 0.0f;
    bool zubaDancando = false;
    ...
    void Iniciar() {
        ...
        AdicionarMensagem(nome, Config.Tr("Sistemas V22 Prontos! Tente meus novos comandos!", "V22 Systems Ready! Try my new commands!"), GOLD);
    }
    ...
    void AtualizarFisica(Vector3 posJogador, Vector3 forwardJogador, float dt) {
        ...
        if (estadoFisico == ORBITAR) { 
            ...
            if (dist > 3.5f) { 
                ...
                if(zubaGiroVelocidade == 0) anguloRotacao = atan2(diff.x, diff.z) * RAD2DEG;
                isMoving = true;
            } else if(zubaGiroVelocidade == 0 && !zubaDancando) {
            ...
            if(zubaGiroVelocidade == 0 && !zubaDancando) { Vector3 dirOlhar = Vector3Subtract(posJogador, posicao); anguloRotacao = atan2(dirOlhar.x, dirOlhar.z) * RAD2DEG; }
        } 
        ...
        if (temModelo && animCount > 0) {
            int currentAnimIndex = (isMoving || zubaDancando) ? ((animCount > 1) ? 1 : 0) : 0; 
            if (isMoving || zubaDancando) animFrame += (int)(std::max(velZuba, 10.0f) * 2.5f); else animFrame = 0;
    ...
    void Desenhar3D() {
        ...
        if (camuflagemAtiva) DrawSphereWires(posicao, r, 16, 16, Fade(BLUE, 0.2f)); 
            ...
            else if (formaAtual == "TORUS") { DrawCylinder({posicao.x, posicao.y + r, posicao.z}, r*1.5f, r*1.5f, r*0.5f, 16, coreColor); DrawCylinderWires({posicao.x, posicao.y + r, posicao.z}, r*1.5f, r*1.5f, r*0.5f, 16, ORANGE); } // Pseudo-torus
            else if (formaAtual == "ACHATADO") { DrawSphereEx({posicao.x, posicao.y + r*0.3f, posicao.z}, r*1.5f, 16, 16, coreColor); } // Achatar escala manual no primitivo não rola fácil, mas fazemos uma esfera larga
            else { 
                if (temModelo) {
...
void DrawCreditsMenu(int larguraTela, int alturaTela, Font font, GameState& estadoAtual, float dt) {
    ...
    DrawTextCenter("V22.0 - THE OMNISCIENCE EXPANSION", startY + 60, 20, LIGHTGRAY);
...
// ==============================================================================
int main(void) {
    InitWindow(1920, 1080, "SUPERGEMINI CREATOR ENGINE - V22.0 THE OMNISCIENCE EXPANSION");
    ToggleFullscreen(); 
    ...
    // NOVO: SISTEMA DE HISTÓRICO DE COMANDOS DO JOGADOR
    ...
    while (!WindowShouldClose() && !sairDoJogo) {
        ...
        if (estadoAtual == MENU) {
            ...
            DrawTextCenter("V22.0 THE OMNISCIENCE EXPANSION", alturaTela/2 - 180, 20, LIGHTGRAY);
            ...
            // CORREÇÃO DA LETRA 'T': Limpa o buffer imediatamente ao abrir o chat
            if (IsKeyPressed(KEY_T) && !modoChat && !modoDiario) { 
                modoChat = true; EnableCursor(); 
                while(GetCharPressed() > 0) {} // Drena o buffer de caracteres pendentes
                indexHistoricoJogador = historicoJogador.size(); // Reseta o cursor do histórico para o final
            } 
                ...
                if (IsKeyPressed(KEY_ENTER) && !inputAtual.empty()) {
                    ...
                    historicoJogador.push_back(inputAtual); // Salva no histórico
                }
            ...
            if (modoChat) {
                ...
                if (IsKeyPressed(KEY_UP)) {
                    if (indexHistoricoJogador > 0) { indexHistoricoJogador--; inputAtual = historicoJogador[indexHistoricoJogador]; }
                }
            ...
            for (auto& ent : entidadesGlobais) {
                ...
                else if (ent.tipo == 3) { ent.raio += dt * 15.0f; ent.tempoVida -= dt; } // Shockwave
                else if (ent.tipo == 4) { ent.tempoVida -= dt; ent.pos.x = camera.position.x + sin(ent.tempoVida*3.0f)*5.0f; ent.pos.y = camera.position.y + 4.0f + sin(ent.tempoVida*5.0f); ent.pos.z = camera.position.z + cos(ent.tempoVida*3.0f)*5.0f; } // Passaro
                else if (ent.tipo == 8) { ent.tempoVida -= dt; ent.pos.x += sin(ent.tempoVida*2.0f)*0.5f; ent.pos.z += cos(ent.tempoVida*1.5f)*0.5f; } // Dragao
                else if (ent.tipo == 9) { ent.tempoVida -= dt; ent.pos.y += sin(ent.tempoVida*4.0f)*0.05f; } // UFO
                else if (ent.tipo == 10) { ent.tempoVida -= dt; EmitirParticulas(ent.pos, SKYBLUE, 5, 30.0f); if(ent.tempoVida < 3.5f && ent.tempoVida > 3.4f) { for(int dx=-6; dx<=6; dx++) for(int dz=-6; dz<=6; dz++) if(sqrt(dx*dx + dz*dz) < 6) modificacoesAlvo[{round(ent.pos.x)+dx, round(ent.pos.z)+dz}] -= 8.0f; } } // Orbital
            ...
            BeginDrawing();
                ...
                BeginMode3D(renderCam);
                    ...
                    if (climaGlobal == 5) DrawSphere(Vector3Add(camera.position, Vector3Scale({ -0.6f, 0.6f, -0.6f }, rDistFinal * 1.2f)), 20.0f, BLACK); // Eclipse
                    ...
                    for (int x = -rDistFinal; x < rDistFinal; x++) {
                        for (int z = -rDistFinal; z < rDistFinal; z++) {
                            ...
                            if (modoFesta) cS = Color{(unsigned char)((wX*10+GetTime()*100)), (unsigned char)((wZ*10-GetTime()*50)), 150, 255};
                    ...
                    for (const auto& ent : entidadesGlobais) {
                        ...
                        else if (ent.tipo == 2) DrawSphere(ent.pos, ent.raio, RED); 
                        else if (ent.tipo == 3) DrawCylinderWires(ent.pos, ent.raio, ent.raio, 0.5f, 16, WHITE); 
                        else if (ent.tipo == 4) DrawCylinder(ent.pos, 0.0f, 0.5f, 1.0f, 4, YELLOW); 
                        else if (ent.tipo == 5) DrawCylinder(ent.pos, 0.5f, 0.5f, 2.0f, 8, Fade(SKYBLUE, 0.6f)); 
                        ...
                        else if (ent.tipo == 8) { for(int d=0; d<5; d++) DrawSphere({ent.pos.x - d*1.5f, ent.pos.y + (float)sin(GetTime()*5+d)*0.5f, ent.pos.z}, 1.0f - d*0.1f, RED); DrawCube({ent.pos.x+1, ent.pos.y+0.5f, ent.pos.z+0.5f}, 0.5f, 0.5f, 0.5f, YELLOW); DrawCube({ent.pos.x+1, ent.pos.y+0.5f, ent.pos.z-0.5f}, 0.5f, 0.5f, 0.5f, YELLOW); } // Dragao
                        else if (ent.tipo == 9) { DrawCylinder(ent.pos, 5.0f, 5.0f, 1.0f, 16, GRAY); DrawSphere({ent.pos.x, ent.pos.y+0.5f, ent.pos.z}, 2.0f, Fade(SKYBLUE, 0.7f)); DrawCylinderEx(ent.pos, {ent.pos.x, ent.pos.y-10.0f, ent.pos.z}, 4.0f, 8.0f, 16, Fade(GREEN, 0.2f)); } // UFO
                        else if (ent.tipo == 10) { DrawCylinderEx({ent.pos.x, ent.pos.y+100.0f, ent.pos.z}, ent.pos, ent.raio, ent.raio, 16, Fade(SKYBLUE, 0.8f)); DrawSphere(ent.pos, ent.raio*1.5f, WHITE); } // Orbital
                        else if (ent.tipo == 11) DrawSphereWires(ent.pos, ent.raio, 32, 32, Fade(SKYBLUE, 0.3f)); // Domo
                        else if (ent.tipo == 12) DrawCylinderWires(ent.pos, ent.raio, ent.raio, 5.0f, 32, ORANGE); // Anel fogo
                    }
                ...
                if (!modoDiario) {
                    ...
                    if (!chatMinimizado) {
                        ...
                        for (const auto& msg : companheiro.historico) {
                            ...
                            todasLinhasVisuais.push_back({"", 30.0f, BLANK}); // Spacer
                        }
                        int maxLinhasNaTela = 7; // Menos linhas por causa da caixa menor
