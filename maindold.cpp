 ...
 * Física Inteligente, Loja, Habilidades, Configurações e GEMINIGODMODE (Ultrawide)
 */
    ...
    void Step(float dt) {
        for (auto& b : bodies) {
            ...
            if (b->material != MAT_BIRD) {
                if (b->vel.magSq() < 150.0f) b->vel = b->vel * 0.90f; 
                else b->vel = b->vel * 0.995f; 
            } else {
                b->vel = b->vel * 0.998f; 
                // Efeito visual de rotação baseada na velocidade X
                b->angle += b->vel.x * 0.005f; 
            }
...
// ============================================================================
enum GameState { STATE_MENU, STATE_SETTINGS, STATE_IDLE, STATE_SHOP, STATE_DRAGGING, STATE_FLYING, STATE_WAITING, STATE_OVER, STATE_WIN };
...
// Nova estrutura para resolver o bug do rastro permanente
struct TrailNode {
    Vec2 pos;
    float life; // Alpha vai de 1.0 a 0.0
};

// Nuvens para Efeito Parallax
struct Cloud {
    Vec2 pos;
    float scale;
    float speed;
...
public:
    ...
    std::vector<int> birdQueue; 
    std::vector<Particle> particles;
    std::vector<TrailNode> birdTrail;
    std::vector<Cloud> clouds;

    // Ajustado dinamicamente para ultrawide
    int virtualW;
    int virtualH;
    
    // Configurações
    bool hdGraphics = true;
    bool showFPS = false;
    float sunRotation = 0.0f;

    Game(int vW, int vH) : virtualW(vW), virtualH(vH) {
        state = STATE_MENU;
        ...
        for(int i=0; i<6; i++){
            Cloud c;
            c.pos = Vec2(GetRandomValue(0, virtualW), GetRandomValue(50, 300));
            c.scale = GetRandomValue(80, 150) / 100.0f;
            c.speed = GetRandomValue(10, 30) / 100.0f;
            clouds.push_back(c);
        }
    }
    ...
    void InitLevel(int lvl) {
        ...
        float startX = virtualW * 0.50f; // Ajustado para ultrawide
        float endX = virtualW * 0.90f;
        float baseY = virtualH - 50.0f;
        
        int numStructures = std::min(2 + (level / 2), 7);
        float spacing = (endX - startX) / std::max(1, (numStructures - 1));

        for (int i = 0; i < numStructures; i++) {
            float x = startX + (i * spacing) + GetRandomValue(-15, 15);
    ...
    void SpawnBird() {
        ...
        birdTrail.clear(); // Correção: Limpa o rastro antigo antes de um novo tiro
    ...
    void SpawnParticles(Vec2 pos, Color c, int count = 8) {
        for (int i = 0; i < count; i++) {
            ...
            p.rot = GetRandomValue(0, 360);
            particles.push_back(p);
        }
    }

    void Explode(Vec2 pos, float radius, float force) {
        SpawnParticles(pos, ORANGE, 50);
        SpawnParticles(pos, RED, 30);
        SpawnParticles(pos, DARKGRAY, 20);
    ...
    void Update(float dt, Vec2 vMouse, bool isTouched, bool isTapped) {
        ...
        for(auto& c : clouds) {
            c.pos.x += c.speed;
            if(c.pos.x > virtualW + 100) c.pos.x = -100;
        }

        if (state == STATE_MENU) {
            Rectangle playBtn = { (float)virtualW/2 - 100.0f, (float)virtualH/2 + 20.0f, 200.0f, 60.0f };
            Rectangle setBtn  = { (float)virtualW/2 - 100.0f, (float)virtualH/2 + 100.0f, 200.0f, 60.0f };
            if (CheckClick(vMouse, playBtn, isTapped)) { score = 0; coins = 50; InitLevel(1); }
            if (CheckClick(vMouse, setBtn, isTapped)) { state = STATE_SETTINGS; }
            return;
        }

        if (state == STATE_SETTINGS) {
            Rectangle hdBtn   = { (float)virtualW/2 - 150.0f, (float)virtualH/2 - 40.0f, 300.0f, 60.0f };
            Rectangle fpsBtn  = { (float)virtualW/2 - 150.0f, (float)virtualH/2 + 40.0f, 300.0f, 60.0f };
            Rectangle backBtn = { (float)virtualW/2 - 100.0f, (float)virtualH/2 + 150.0f, 200.0f, 50.0f };

            if (CheckClick(vMouse, hdBtn, isTapped)) hdGraphics = !hdGraphics;
            if (CheckClick(vMouse, fpsBtn, isTapped)) showFPS = !showFPS;
            if (CheckClick(vMouse, backBtn, isTapped)) state = STATE_MENU;
            return;
        }

        if (state == STATE_OVER || state == STATE_WIN) {
            Rectangle nextBtn = { (float)virtualW/2 - 150.0f, (float)virtualH/2 + 60.0f, 300.0f, 60.0f };
            if (CheckClick(vMouse, nextBtn, isTapped)) {
                if (state == STATE_OVER) { score = 0; coins = 50; InitLevel(1); }
                else InitLevel(level + 1);
            }
            return;
        }
        ...
        if (state == STATE_SHOP) {
            ...
            if (CheckClick(vMouse, btnYellow, isTapped) && coins >= 50) {
                coins -= 50; 
                // Correção Mágica da Fila!
                if (currentBird) {
                    birdQueue.insert(birdQueue.begin(), currentBird->subType); // Devolve o atual
                    currentBird->isDestroyed = true;
                    currentBird = nullptr;
                }
                birdQueue.insert(birdQueue.begin(), BIRD_YELLOW); 
            }
            if (CheckClick(vMouse, btnBlack, isTapped) && coins >= 100) {
                coins -= 100; 
                if (currentBird) {
                    birdQueue.insert(birdQueue.begin(), currentBird->subType); 
                    currentBird->isDestroyed = true;
                    currentBird = nullptr;
                }
                birdQueue.insert(birdQueue.begin(), BIRD_BLACK); 
            }
            if (CheckClick(vMouse, btnClose, isTapped)) {
                state = STATE_IDLE;
                if (!currentBird) SpawnBird(); 
            }
            return;
        }
        ...
        if (state == STATE_IDLE || state == STATE_DRAGGING) {
            if (isTouched) {
                if (state == STATE_IDLE && currentBird && (vMouse - currentBird->pos).mag() < 90.0f) {
        ...
        if (state == STATE_FLYING && currentBird) {
            if (isTapped && !currentBird->abilityUsed) {
                if (currentBird->subType == BIRD_YELLOW) {
                    currentBird->vel = currentBird->vel * 2.5f; 
                    currentBird->vel.y = 0; 
                    currentBird->abilityUsed = true;
                    SpawnParticles(currentBird->pos, YELLOW, 20);
                } else if (currentBird->subType == BIRD_BLACK) {
                    Explode(currentBird->pos, 220.0f, 1800.0f); 
                    currentBird->isDestroyed = true;
                }
            }

            if (!currentBird->isDestroyed) {
                if (currentBird->vel.magSq() > 100.0f && GetRandomValue(0,100) > 40) {
                    // Adicionando rastro com FadeOut
                    birdTrail.push_back({currentBird->pos, 1.0f});
                }

                bool stopped = currentBird->vel.magSq() < 10.0f;
                bool outOfBounds = currentBird->pos.x > virtualW + 200 || currentBird->pos.x < -200;
        ...
        // Atualização Suave do Rastro (Fade out perfeito!)
        for (auto& t : birdTrail) {
            t.life -= dt * 1.5f;
        }
        birdTrail.erase(std::remove_if(birdTrail.begin(), birdTrail.end(), [](const TrailNode& t) { return t.life <= 0; }), birdTrail.end());
        ...
        if (state == STATE_WAITING) {
            ...
            if (timer > 2.0f) {
                ...
                currentBird = nullptr;
                birdTrail.clear(); // Limpeza de segurança extra
        ...
        for (auto& p : particles) {
            p.pos += p.vel;
            p.vel.y += 0.5f;
            p.rot += p.vel.x * 2.0f; // GodMode: Rotação nas partículas
            p.life -= dt * 1.2f;
        }
        ...
        if (pigsLeft <= 0 && state != STATE_WIN) {
            ...
            coins += birdQueue.size() * 20; 
        }
    }

    void Draw() {
        // Céu com gradiente (Simulado com retângulos largos)
        DrawRectangle(0, 0, virtualW, virtualH/2, Color{ 100, 180, 255, 255 });
        DrawRectangle(0, virtualH/2, virtualW, virtualH/2, Color{ 135, 206, 250, 255 });

        // Sol e Raios GodMode
        DrawPoly(Vector2{(float)virtualW * 0.85f, (float)virtualH * 0.2f}, 12, 80, sunRotation, Color{255, 215, 0, 100});
        DrawCircle(virtualW * 0.85f, virtualH * 0.2f, 60, GOLD);
        
        // Nuvens Parallax
        for(const auto& c : clouds) {
            DrawCircle(c.pos.x, c.pos.y, 40 * c.scale, Color{255,255,255,200});
            DrawCircle(c.pos.x + 30*c.scale, c.pos.y - 10*c.scale, 50 * c.scale, Color{255,255,255,200});
            DrawCircle(c.pos.x + 60*c.scale, c.pos.y, 40 * c.scale, Color{255,255,255,200});
        }

        // Rastro com Fade
        for (const auto& t : birdTrail) {
            unsigned char alpha = (unsigned char)(255 * t.life);
            DrawCircleV(Vector2{t.pos.x, t.pos.y}, 5, Color{255, 255, 255, alpha});
        }

        // Haste Traseira
        DrawRectangle(slingAnchor.x + 5, slingAnchor.y - 20, 15, virtualH - slingAnchor.y, Color{139, 69, 19, 255});

        // Sombras Dinâmicas no Chão (GodMode Gráfico)
        if (hdGraphics) {
            for (const auto& b : world.bodies) {
                if (!b->isStatic) {
                    float shadowW = (b->shape == SHAPE_BOX) ? b->width : b->radius * 2;
                    DrawEllipse(b->pos.x, virtualH - 25.0f, shadowW * 0.6f, 6.0f, Color{0, 0, 0, 60});
                }
            }
        }
        ...
        for (const auto& b : world.bodies) {
            if (b->shape == SHAPE_BOX) {
                Rectangle rec = { b->pos.x, b->pos.y, b->width, b->height };
                Color c = b->isDamaged ? ColorBrightness(b->color, -0.3f) : b->color;
                
                // Rotaciona a caixa usando o sistema Pro
                DrawRectanglePro(rec, Vector2{b->width/2, b->height/2}, b->angle * RAD2DEG, c);
                
                // Texturas HD
                if (hdGraphics) {
                    if (b->material == MAT_WOOD && !b->isStatic) {
                        DrawLineEx(Vector2{b->pos.x - b->width/3, b->pos.y - b->height/2}, Vector2{b->pos.x - b->width/3, b->pos.y + b->height/2}, 2, ColorBrightness(c, -0.4f));
                        DrawLineEx(Vector2{b->pos.x + b->width/3, b->pos.y - b->height/2}, Vector2{b->pos.x + b->width/3, b->pos.y + b->height/2}, 2, ColorBrightness(c, -0.4f));
                    }
                    if (b->material == MAT_GLASS) {
                        DrawRectanglePro(rec, Vector2{b->width/2, b->height/2}, b->angle * RAD2DEG, Color{255,255,255,80});
                    }
                }
                DrawRectanglePro(rec, Vector2{b->width/2, b->height/2}, b->angle * RAD2DEG, Color{0,0,0,0}); // Linhas (Sem fill direto aqui)

            } else if (b->shape == SHAPE_CIRCLE) {
                Color c = b->isDamaged ? ColorBrightness(b->color, -0.3f) : b->color;
                
                // Pássaro Amarelo Triângulo
                if (b->material == MAT_BIRD && b->subType == BIRD_YELLOW) {
                    DrawPoly(Vector2{b->pos.x, b->pos.y}, 3, b->radius + 5, (b->angle * RAD2DEG) + 30.0f, c);
                    DrawCircle(b->pos.x + 5, b->pos.y - 2, 4, WHITE);
                    DrawCircle(b->pos.x + 6, b->pos.y - 2, 2, BLACK);
                } else {
                    ...
                    if (b->material == MAT_BIRD) { 
                        ...
                        else if (b->subType == BIRD_BLACK) DrawRectangle(b->pos.x-3, b->pos.y-b->radius-8, 6, 8, ORANGE);
                        ...
                        if (b->subType == PIG_HELMET) DrawCircleSector(Vector2{b->pos.x, b->pos.y}, b->radius, 180, 360, 0, GRAY);
                        else if (b->subType == PIG_KING) DrawRectangle(b->pos.x - 10, b->pos.y - b->radius - 15, 20, 15, GOLD);
                    }
                }
            }
        }

        // Couro e Elásticos do Estilingue (GodMode)
        if (state == STATE_DRAGGING && currentBird) {
            ...
            DrawLineEx(Vector2{slingAnchor.x + 10, slingAnchor.y - 10}, Vector2{currentBird->pos.x, currentBird->pos.y}, 6, Color{62, 39, 35, 255});
            DrawRectangle(currentBird->pos.x - 8, currentBird->pos.y - 10, 16, 20, Color{101, 67, 33, 255}); // Couro
        }
        ...
        for (const auto& p : particles) {
            ...
            if (hdGraphics) {
                DrawPoly(Vector2{p.pos.x, p.pos.y}, 4, p.size, p.rot, pc);
            } else {
        ...
        // ================= UI e MODAIS =================
        if (state == STATE_MENU) {
            DrawRectangle(0, 0, virtualW, virtualH, Color{0,0,0,180});
            DrawText("PASSAROS FURIOSOS", virtualW/2 - MeasureText("PASSAROS FURIOSOS", 60)/2, virtualH/2 - 120, 60, GOLD);
            
            DrawRectangle(virtualW/2 - 100, virtualH/2 + 20, 200, 60, GREEN);
            DrawText("JOGAR", virtualW/2 - MeasureText("JOGAR", 30)/2, virtualH/2 + 35, 30, WHITE);
            
            DrawRectangle(virtualW/2 - 100, virtualH/2 + 100, 200, 60, DARKGRAY);
            DrawText("OPCOES", virtualW/2 - MeasureText("OPCOES", 30)/2, virtualH/2 + 115, 30, WHITE);
            return;
        }

        if (state == STATE_SETTINGS) {
            DrawRectangle(0, 0, virtualW, virtualH, Color{0,0,0,220});
            DrawText("CONFIGURACOES", virtualW/2 - MeasureText("CONFIGURACOES", 50)/2, 100, 50, ORANGE);

            DrawRectangle(virtualW/2 - 150, virtualH/2 - 40, 300, 60, hdGraphics ? GREEN : RED);
            DrawText(TextFormat("GRAFICOS HD: %s", hdGraphics ? "LIGADO" : "DESLIGADO"), virtualW/2 - 130, virtualH/2 - 25, 25, WHITE);

            DrawRectangle(virtualW/2 - 150, virtualH/2 + 40, 300, 60, showFPS ? GREEN : RED);
            DrawText(TextFormat("EXIBIR FPS: %s", showFPS ? "LIGADO" : "DESLIGADO"), virtualW/2 - 130, virtualH/2 + 55, 25, WHITE);

            DrawRectangle(virtualW/2 - 100, virtualH/2 + 150, 200, 50, GRAY);
            DrawText("VOLTAR", virtualW/2 - MeasureText("VOLTAR", 25)/2, virtualH/2 + 162, 25, WHITE);
            return;
        }
        ...
        if (showFPS) DrawFPS(20, virtualH - 40);
    ...
    void DrawModal(const char* title, const char* subtitle) {
        ...
        DrawRectangle(virtualW/2 - 300, virtualH/2 - 150, 600, 300, GOLD);
        DrawRectangleLines(virtualW/2 - 300, virtualH/2 - 150, 600, 300, WHITE);
        ...
        // Botão integrado ao Modal
        DrawRectangle(virtualW/2 - 150, virtualH/2 + 60, 300, 60, GREEN);
        DrawText("CONTINUAR", virtualW/2 - MeasureText("CONTINUAR", 25)/2, virtualH/2 + 77, 25, WHITE);
    }
};

// ============================================================================
// 4. MAIN LOOP - GodMode Auto-Resolution Ultrawide
// ============================================================================
int main() {
    SetConfigFlags(FLAG_WINDOW_RESIZABLE | FLAG_VSYNC_HINT | FLAG_MSAA_4X_HINT); // MSAA ativado para bordas perfeitas!
    InitWindow(1280, 720, "Passaros Furiosos (GodMode)");
    SetTargetFPS(60);

    // Detetar Proporção Ultrawide (Ex: 2400x1080) e expandir as laterais do jogo!
    int screenW = GetScreenWidth();
    int screenH = GetScreenHeight();
    float aspect = (screenH == 0) ? (16.0f/9.0f) : ((float)screenW / (float)screenH);
    
    int internalH = 720;
    int internalW = std::max(1280, (int)(internalH * aspect));

    Game game(internalW, internalH);
