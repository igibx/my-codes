// ========================================================================================
// SUPERGEMINI CREATOR ENGINE - V25.0 (THE FOX SUPREMACY & WILDLIFE POLISH)
// Architect: Gemini God Mode | Executive Producer: Igor Bettarello Xavier (OMEGARED)
// Features: Zuba Removed, Player Inherits ALL God Powers/NLP, Fixed Animals Orientation/Culling.
// ========================================================================================
...
void EmitirParticulas(Vector3 pos, Color cor, int qtd, float velMax) {
    for(int i=0; i<qtd; i++) {
        ...
        particulas.push_back(Particula{pos, v, cor, 1.0f + RandFloat(), 1.0f + RandFloat(), RandFloat()*0.3f + 0.1f});
    }
}
...
// ==============================================================================
// ANIMAL MANAGER (Gerencia a vida selvagem dinamicamente)
// ==============================================================================
struct AnimalInst {
    int tipo; Vector3 pos; Vector3 alvo; float angulo; float vel; float escala;
};
...
public:
    ...
    std::vector<AnimalInst> instances;
    
    const char* paths[7] = {
        "src/animals/glb/butterfly.glb",
        "src/animals/glb/cat.glb",
        "src/animals/glb/chichen.glb",
        "src/animals/glb/Cow.glb",
        "src/animals/glb/dog.glb",
        "src/animals/glb/girafe.glb",
        "src/animals/glb/ladybug.glb"
    };
    float scales[7] = { 0.4f, 0.8f, 0.6f, 1.5f, 0.9f, 2.5f, 0.2f };
    float speeds[7] = { 8.0f, 4.0f, 2.5f, 1.5f, 5.0f, 2.0f, 0.5f };

    void Init(Shader shaderLuz) {
        for (int i = 0; i < 7; i++) {
            if (FileExists(paths[i])) {
                Model m = LoadModel(paths[i]);
                for (int j = 0; j < m.materialCount; j++) m.materials[j].shader = shaderLuz;
                models.push_back(m);
                
                int count = 0;
                ModelAnimation* anim = LoadModelAnimations(paths[i], &count);
                anims.push_back(anim);
                animCounts.push_back(count);
                animFrames.push_back(0.0f);
            } else {
                models.push_back({0}); anims.push_back(nullptr); animCounts.push_back(0); animFrames.push_back(0.0f);
            }
        }
        for(int i=0; i<80; i++) SpawnAnimal((Vector3){ (RandFloat()-0.5f)*200.0f, 0, (RandFloat()-0.5f)*200.0f });
    }
    
    void SpawnAnimal(Vector3 around) {
        AnimalInst a; a.tipo = rand() % 7;
        a.pos = (Vector3){ around.x + (RandFloat()-0.5f)*80.0f, 0, around.z + (RandFloat()-0.5f)*80.0f };
        a.alvo = (Vector3){ a.pos.x + (RandFloat()-0.5f)*40.0f, 0, a.pos.z + (RandFloat()-0.5f)*40.0f };
        a.angulo = 0.0f; a.vel = speeds[a.tipo] * (0.8f + RandFloat()*0.4f); a.escala = scales[a.tipo];
    ...
    void Update(float dt, Vector3 playerPos) {
        for(size_t i=0; i<models.size(); i++) {
            if (animCounts[i] > 0) {
                animFrames[i] += dt * 60.0f;
                if (animFrames[i] >= anims[i][0].frameCount) animFrames[i] = 0;
                UpdateModelAnimation(models[i], anims[i][0], (int)animFrames[i]);
            }
        }
        for(auto& a : instances) {
            if (Vector3Distance(a.pos, playerPos) > 150.0f) {
                a.pos = (Vector3){ playerPos.x + (RandFloat()-0.5f)*120.0f, 0, playerPos.z + (RandFloat()-0.5f)*120.0f };
                a.alvo = (Vector3){ a.pos.x + (RandFloat()-0.5f)*40.0f, 0, a.pos.z + (RandFloat()-0.5f)*40.0f };
            }
            Vector3 diff = Vector3Subtract(a.alvo, a.pos); diff.y = 0; float dist = Vector3Length(diff);
            if (dist < 2.0f) a.alvo = (Vector3){ a.pos.x + (RandFloat()-0.5f)*40.0f, 0, a.pos.z + (RandFloat()-0.5f)*40.0f };
            else {
                diff = Vector3Normalize(diff);
                a.pos.x += diff.x * a.vel * dt; a.pos.z += diff.z * a.vel * dt;
                
                // FIX: Adicionado +180 graus para eles andarem de frente!
                float targetAngle = atan2(diff.x, diff.z) * RAD2DEG + 180.0f;
                float deltaAngulo = targetAngle - a.angulo;
                while (deltaAngulo > 180.0f) deltaAngulo -= 360.0f;
                while (deltaAngulo < -180.0f) deltaAngulo += 360.0f;
                a.angulo += deltaAngulo * dt * 5.0f;
            }
            mundoMutex.lock(); float chao = LerAlturaMundo(a.pos.x, a.pos.z); mundoMutex.unlock();
            if (a.tipo == 0) a.pos.y = Lerp(a.pos.y, chao + 2.0f + sin(GetTime()*3.0f + a.pos.x)*0.5f, dt * 2.0f); // Borboleta
            else a.pos.y = chao;
        }
    }

    void Draw(Vector3 cameraPos, float limitDist) {
        for (auto& a : instances) {
            // FIX: Culling de distância. Não desenha animais flutuando além do limite do chão!
            if (Vector3Distance(a.pos, cameraPos) < limitDist - 5.0f) {
                if (models[a.tipo].meshCount > 0) {
                    DrawModelEx(models[a.tipo], a.pos, {0,1,0}, a.angulo, {a.escala, a.escala, a.escala}, WHITE);
                }
            }
        }
    }
    ...
    void Unload() {
        for (size_t i=0; i<models.size(); i++) {
            if (models[i].meshCount > 0) UnloadModel(models[i]);
            if (animCounts[i] > 0) UnloadModelAnimations(anims[i], animCounts[i]);
        }
    }
};
...
// ==============================================================================
// 3. IA NLP AVANÇADA (AGORA PROCESSADA DIRETAMENTE PELA FOX/SISTEMA)
// ==============================================================================
...
RespostaIA ProcessarIAInterna(std::string inputJogador, float alturaAtual) {
    ...
    if (Match(in, {"cubo", "quadrad", "caixa"})) { cmds["FORMA"] = isOff ? "FOX" : "CUBO"; resp += isOff ? Config.Tr("Retornando a perfeicao vulpina. ", "Returning to vulpine perfection. ") : Config.Tr("Fox Cubica! ", "Cubic Fox! "); acted = true; } 
    else if (Match(in, {"piramide", "triangulo"}) && !Match(in, {"constru", "faca"})) { cmds["FORMA"] = isOff ? "FOX" : "PIRAMIDE"; resp += isOff ? Config.Tr("Normal. ", "Normal. ") : Config.Tr("Fox Piramidal. ", "Pyramidal Fox. "); acted = true; } 
    ...
    else if (Match(in, {"achatad", "panqueca", "pizza"})) { cmds["FORMA"] = "ACHATADO"; resp += Config.Tr("Fui esmagada! ", "I got squashed! "); acted = true; }
    else if (Match(in, {"esfera", "bola", "redond"})) { cmds["FORMA"] = "ESFERA"; resp += Config.Tr("Forma esferica perfeita. ", "Perfect spherical form. "); acted = true; }
    else if (Match(in, {"fox", "raposa"})) { cmds["FORMA"] = "FOX"; resp += Config.Tr("Retornando a forma de Raposa! ", "Returning to Fox form! "); acted = true; }

    if (Match(in, {"gigante", "imenso", "cresca", "enorme"})) { cmds["TAMANHO"] = isOff ? "NORMAL" : "GIGANTE"; resp += isOff ? Config.Tr("Fox encolhendo. ", "Fox shrinking. ") : Config.Tr("Fox GIGANTE! ", "GIANT Fox! "); acted = true; } 
    else if (Match(in, {"pequen", "minúscul", "diminua", "encolh"})) { cmds["TAMANHO"] = isOff ? "NORMAL" : "PEQUENO"; resp += isOff ? Config.Tr("Tamanho restaurado. ", "Size restored. ") : Config.Tr("Mini Fox. ", "Mini Fox. "); acted = true; }
    ...
    if (Match(in, {"gire", "roda", "piao"})) { cmds["ACAO"] = "GIRAR"; resp += Config.Tr("Estou tonta! ", "I'm dizzy! "); acted = true; }
    if (Match(in, {"pule", "salta", "voe"})) { cmds["ACAO"] = "PULAR"; resp += Config.Tr("Boing! ", "Boing! "); acted = true; }
    if (Match(in, {"dance", "rebola"})) { cmds["ACAO"] = "DANCAR"; resp += Config.Tr("Olha o meu passinho! ", "Watch my moves! "); acted = true; }
    if (Match(in, {"segu", "volta", "frente", "explore", "para", "parad", "fique ai"})) { cmds["IGNORAR"] = "IGNORAR"; resp += Config.Tr("Eu mesma ando agora! ", "I walk by myself now! "); acted = true; }
    ...
    if (Match(in, {"constru", "faca", "crie"}) && Match(in, {"piramide"})) { cmds["CONSTRUCAO"] = "PIRAMIDE"; resp += Config.Tr("Fox Farao! ", "Pharaoh Fox! "); acted = true; }
    ...
    if (!acted) {
        if (Match(in, {"ola", "oi", "bom dia"})) resp = Config.Tr("Oieee! Eu sou a Fox Magica! O que vamos fazer?", "Hiii! I'm the Magic Fox! What are we doing?");
...
// ==============================================================================
// 4. O JOGADOR (FÍSICA, ANIMAÇÃO, FORMAS, CHAT E MÁGICA)
// ==============================================================================
...
public:
    ...
    bool temModelo;
    std::string formaAtual; 

    float escalaAtual; float escalaAlvo;
    float puloTimer; float giroVelocidade; bool dancando;
    float tempoLaser; Vector3 alvoLaser; std::string itemEquipado; 
    
    std::vector<MensagemChat> historico; std::mutex chatMutex;
    std::atomic<bool> pensando; std::atomic<bool> forcadoScroll; 

    void Init() {
        posicao = (Vector3){0, 15, 0};
        anguloRotacao = 0.0f;
        temModelo = false;
        animCount = 0;
        ...
        formaAtual = "FOX";
        escalaAtual = 1.0f; escalaAlvo = 1.0f;
        puloTimer = 0.0f; giroVelocidade = 0.0f; dancando = false;
        tempoLaser = 0.0f; itemEquipado = "";
        pensando = false; forcadoScroll = false;
        
        historico.clear();
        AdicionarMensagem("Sistema", Config.Tr("Sistemas V25 Prontos! Fox assumiu o controle Divino!", "V25 Systems Ready! Fox has taken Divine control!"), GOLD);
    }
    ...
    void ProcessarConversa(std::string inputJogador, float alturaAtual, Vector3 forwardJogador) {
        AdicionarMensagem(Config.Tr("Voce", "You"), inputJogador, LIGHTGRAY); pensando = true;
        
        std::thread([this, inputJogador, alturaAtual, forwardJogador]() {
            ...
            if (cmds["ACAO"] == "ATIRAR") {
                tempoLaser = 2.5f; 
                alvoLaser = Vector3Add(posicao, Vector3Scale(forwardJogador, 30.0f)); 
                mundoMutex.lock(); alvoLaser.y = LerAlturaMundo(alvoLaser.x, alvoLaser.z) + 1.0f; mundoMutex.unlock(); 
            } 
            else if (cmds["ACAO"] == "PULAR") puloTimer = 1.0f;
            else if (cmds["ACAO"] == "GIRAR") giroVelocidade = 1000.0f;
            else if (cmds["ACAO"] == "DANCAR") dancando = !dancando;
            ...
            if (cmds["ATAQUE"] == "BURACO_NEGRO") { Vector3 dest = Vector3Add(posicao, Vector3Scale(forwardJogador, 40.0f)); mundoMutex.lock(); entidadesGlobais.push_back(EntidadeDivina{1, dest, (Vector3){0,0,0}, 10.0f, 10.0f}); mundoMutex.unlock(); cameraShake = 2.0f; } 
            else if (cmds["ATAQUE"] == "METEOROS") { mundoMutex.lock(); for (int i=0; i<5; i++) entidadesGlobais.push_back(EntidadeDivina{2, (Vector3){posicao.x + (rand()%60 - 30), posicao.y + 80.0f + (i*10), posicao.z + (rand()%60 - 30)}, (Vector3){0, -50.0f, 0}, 3.0f, 5.0f}); mundoMutex.unlock(); } 
            else if (cmds["ATAQUE"] == "ONDA_CHOQUE") { mundoMutex.lock(); entidadesGlobais.push_back(EntidadeDivina{3, posicao, (Vector3){0,0,0}, 0.5f, 3.0f}); mundoMutex.unlock(); cameraShake = 1.0f; }
            else if (cmds["ATAQUE"] == "ORBITAL") { Vector3 dest = Vector3Add(posicao, Vector3Scale(forwardJogador, 20.0f)); mundoMutex.lock(); entidadesGlobais.push_back(EntidadeDivina{10, dest, (Vector3){0,0,0}, 5.0f, 4.0f}); mundoMutex.unlock(); cameraShake = 3.0f; }
            ...
            if (cmds["DEFESA"] != "") {
                ...
                else if (cmds["DEFESA"] == "DOMO") { Vector3 dest = posicao; mundoMutex.lock(); entidadesGlobais.push_back(EntidadeDivina{11, dest, (Vector3){0,0,0}, 20.0f, 30.0f}); mundoMutex.unlock(); }
                else if (cmds["DEFESA"] == "ANEL_FOGO") { Vector3 dest = posicao; mundoMutex.lock(); entidadesGlobais.push_back(EntidadeDivina{12, dest, (Vector3){0,0,0}, 10.0f, 30.0f}); mundoMutex.unlock(); }
            }
            
            if (cmds["INVOCAR"] == "PASSARO") { mundoMutex.lock(); entidadesGlobais.push_back(EntidadeDivina{4, (Vector3){posicao.x, posicao.y + 10.0f, posicao.z}, (Vector3){0,0,0}, 0.5f, 60.0f}); mundoMutex.unlock(); } 
            else if (cmds["INVOCAR"] == "GOLEM") { Vector3 dest = Vector3Add(posicao, Vector3Scale(forwardJogador, 10.0f)); mundoMutex.lock(); entidadesGlobais.push_back(EntidadeDivina{6, dest, (Vector3){0,0,0}, 1.0f, 60.0f}); mundoMutex.unlock(); } 
            else if (cmds["INVOCAR"] == "DRAGAO") { mundoMutex.lock(); entidadesGlobais.push_back(EntidadeDivina{8, (Vector3){posicao.x, posicao.y + 30.0f, posicao.z}, (Vector3){0,0,0}, 2.0f, 60.0f}); mundoMutex.unlock(); }
            else if (cmds["INVOCAR"] == "UFO") { Vector3 dest = (Vector3){posicao.x, posicao.y + 20.0f, posicao.z}; mundoMutex.lock(); entidadesGlobais.push_back(EntidadeDivina{9, dest, (Vector3){0,0,0}, 5.0f, 60.0f}); mundoMutex.unlock(); }
            ...
            if (cmds["TERRENO"] != "") {
                Vector3 alvo = Vector3Add(posicao, Vector3Scale(forwardJogador, 20.0f)); int cx = round(alvo.x); int cz = round(alvo.z);
                ...
                else if (cmds["TERRENO"] == "LAGO") { for(int dx=-8; dx<=8; dx++) for(int dz=-8; dz<=8; dz++) if(sqrt(dx*dx + dz*dz) < 8) { modificacoesAlvo[{cx+dx, cz+dz}] -= 4.0f; filaConstrucao.push_back(BlocoFixo{(Vector3){ (float)cx+dx, GerarAlturaTerrenoBase(cx, cz)-1.0f, (float)cz+dz }, BLUE}); } }
            ...
            if (cmds["CONSTRUCAO"] != "") {
                ...
                if (cmds["CONSTRUCAO"] == "PIRAMIDE") { for (int y=0; y<8; y++) for (int x=-8+y; x<=8-y; x++) for (int z=-8+y; z<=8-y; z++) filaConstrucao.push_back(BlocoFixo{(Vector3){ (float)cx+x, bA+y, (float)cz+z }, GOLD}); } 
                else if (cmds["CONSTRUCAO"] == "TORRE") { for (int y=0; y<15; y++) for (int x=-2; x<=2; x++) for (int z=-2; z<=2; z++) if (x==-2||x==2||z==-2||z==2||y==14) filaConstrucao.push_back(BlocoFixo{(Vector3){ (float)cx+x, bA+y, (float)cz+z }, DARKGRAY}); } 
                else if (cmds["CONSTRUCAO"] == "PONTE") { for (int d=-10; d<=10; d++) for (int w=-1; w<=1; w++) filaConstrucao.push_back(BlocoFixo{(Vector3){ (float)cx+d, bA, (float)cz+w }, BROWN}); } 
                else if (cmds["CONSTRUCAO"] == "MURALHA") { for (int d=-15; d<=15; d++) for (int y=0; y<5; y++) filaConstrucao.push_back(BlocoFixo{(Vector3){ (float)cx+d, bA+y, (float)cz }, LIGHTGRAY}); } 
                else if (cmds["CONSTRUCAO"] == "CASA") { for (int y=0; y<4; y++) for (int x=-3; x<=3; x++) for (int z=-3; z<=3; z++) if (y==3||x==-3||x==3||z==-3||z==3) { if (y<2&&z==3&&x>=-1&&x<=1) continue; filaConstrucao.push_back(BlocoFixo{(Vector3){ (float)cx+x, bA+y, (float)cz+z }, ORANGE}); } }
                else if (cmds["CONSTRUCAO"] == "TEMPLO") { for(int y=0;y<6;y++) for(int x=-5;x<=5;x+=5) for(int z=-5;z<=5;z+=5) filaConstrucao.push_back(BlocoFixo{(Vector3){(float)cx+x, bA+y, (float)cz+z}, WHITE}); for(int x=-6;x<=6;x++) for(int z=-6;z<=6;z++) filaConstrucao.push_back(BlocoFixo{(Vector3){(float)cx+x, bA+6, (float)cz+z}, GOLD}); }
                else if (cmds["CONSTRUCAO"] == "MONOLITO") { for(int y=0;y<20;y++) for(int x=-1;x<=1;x++) for(int z=-1;z<=1;z++) filaConstrucao.push_back(BlocoFixo{(Vector3){(float)cx+x, bA+y, (float)cz+z}, BLACK}); }
            }

            AdicionarMensagem("Sistema Fox", ia.texto, GOLD); pensando = false;
        }).detach();
    }
    ...
    void Atualizar(float dt, Vector3 moveDir, float speedMultReal, bool isJumping) {
        ...
        if (puloTimer > 0) puloTimer -= dt * 2.0f;
        if (giroVelocidade > 0) { anguloRotacao += giroVelocidade * dt; giroVelocidade -= dt * 100.0f; if (giroVelocidade < 0) giroVelocidade = 0; }
        if (dancando) { anguloRotacao += sin(GetTime() * 5.0f) * 15.0f; }
        
        if (tempoLaser > 0) { 
            tempoLaser -= dt; 
            mundoMutex.lock(); EmitirParticulas(alvoLaser, ORANGE, 5, 5.0f); mundoMutex.unlock();
        }

        // Animator
        if (temModelo && animCount > 0) {
            int nextAnim = 0;
            if (isJumping || puloTimer > 0) nextAnim = (animCount > 3) ? 3 : 0; 
            else if (isMoving) {
                if (speedMultReal > 1.2f) nextAnim = (animCount > 2) ? 2 : ((animCount > 1) ? 1 : 0); 
                else nextAnim = (animCount > 1) ? 1 : 0; 
            } else if (dancando) nextAnim = (animCount > 2) ? 2 : 0;
    ...
    void Desenhar(bool isGodMode, Shader shaderLuz, Vector3 cameraPos) {
        float r = 0.8f * escalaAtual;
        
        float cpos[3] = { cameraPos.x, cameraPos.y, cameraPos.z };
        SetShaderValue(shaderLuz, GetShaderLocation(shaderLuz, "viewPos"), cpos, SHADER_UNIFORM_VEC3);
        
        float puloY = (puloTimer > 0) ? sin(puloTimer * PI) * 5.0f * escalaAtual : 0.0f;
        if (dancando) puloY += abs(sin(GetTime() * 10.0f)) * 1.5f;

        Vector3 renderPos = (Vector3){posicao.x, posicao.y + puloY, posicao.z};
        ...
        if (camuflagemAtiva) DrawSphereWires(renderPos, r, 16, 16, Fade(BLUE, 0.2f)); 
        else {
            if (formaAtual == "FOX" && temModelo) {
                DrawModelEx(modeloFox, (Vector3){renderPos.x, alturaSombra, renderPos.z}, (Vector3){0,1,0}, anguloRotacao, (Vector3){escalaAtual, 0.01f, escalaAtual}, Fade(BLACK, 0.6f));
                DrawModelEx(modeloFox, renderPos, (Vector3){0,1,0}, anguloRotacao, (Vector3){escalaAtual, escalaAtual, escalaAtual}, isGodMode ? Fade(SKYBLUE, 0.8f) : WHITE);
            } else {
                DrawCylinder((Vector3){renderPos.x, alturaSombra, renderPos.z}, r, r, 0.05f, 16, Fade(BLACK, 0.6f));
                if (formaAtual == "CUBO") DrawCube((Vector3){renderPos.x, renderPos.y + r, renderPos.z}, r*2.2f, r*2.2f, r*2.2f, coreColor);
                else if (formaAtual == "PIRAMIDE") DrawCylinder((Vector3){renderPos.x, renderPos.y, renderPos.z}, 0.0f, r*1.8f, r*3.0f, 4, coreColor);
                else if (formaAtual == "CILINDRO") DrawCylinder((Vector3){renderPos.x, renderPos.y + r, renderPos.z}, r*1.5f, r*1.5f, r*3.0f, 16, coreColor);
                else if (formaAtual == "TORUS") { DrawCylinder((Vector3){renderPos.x, renderPos.y + r, renderPos.z}, r*1.5f, r*1.5f, r*0.5f, 16, coreColor); DrawCylinderWires((Vector3){renderPos.x, renderPos.y + r, renderPos.z}, r*1.5f, r*1.5f, r*0.5f, 16, ORANGE); }
                else if (formaAtual == "ACHATADO") { DrawSphereEx((Vector3){renderPos.x, renderPos.y + r*0.3f, renderPos.z}, r*1.5f, 16, 16, coreColor); } 
                else DrawSphere((Vector3){renderPos.x, renderPos.y + r, renderPos.z}, r, coreColor);
            }
        }

        if (!camuflagemAtiva && itemEquipado != "") {
            Vector3 handPos = {renderPos.x + r*1.5f, renderPos.y + r*1.5f, renderPos.z};
            if (itemEquipado == "ESPADA") { DrawCube(handPos, 0.2f, r*3.0f, 0.2f, LIGHTGRAY); DrawCube((Vector3){handPos.x, handPos.y - r*1.0f, handPos.z}, 0.8f, 0.2f, 0.2f, BROWN); } 
            else if (itemEquipado == "PICARETA") { DrawCube(handPos, 0.2f, r*3.0f, 0.2f, BROWN); DrawCube((Vector3){handPos.x, handPos.y + r*1.0f, handPos.z}, r*2.0f, 0.2f, 0.2f, GRAY); } 
            else if (itemEquipado == "ARCO") { DrawCylinderEx((Vector3){handPos.x, handPos.y - r*1.0f, handPos.z}, (Vector3){handPos.x, handPos.y + r*1.0f, handPos.z}, 0.1f, 0.1f, 8, BROWN); DrawLine3D((Vector3){handPos.x, handPos.y - r*1.0f, handPos.z}, (Vector3){handPos.x, handPos.y + r*1.0f, handPos.z}, WHITE); }
            else if (itemEquipado == "VARINHA") { DrawCylinderEx((Vector3){handPos.x, handPos.y - r*1.0f, handPos.z}, (Vector3){handPos.x, handPos.y + r*1.0f, handPos.z}, 0.1f, 0.1f, 8, BLACK); DrawSphere((Vector3){handPos.x, handPos.y + r*1.2f, handPos.z}, 0.3f, PURPLE); }
            else if (itemEquipado == "LANTERNA") { DrawCube(handPos, 0.5f, 0.8f, 0.5f, DARKGRAY); DrawCube((Vector3){handPos.x, handPos.y, handPos.z}, 0.4f, 0.6f, 0.4f, YELLOW); }
            else if (itemEquipado == "ESCUDO_ITEM") { DrawCylinderEx((Vector3){handPos.x, handPos.y, handPos.z}, (Vector3){handPos.x+0.1f, handPos.y, handPos.z}, 1.5f, 1.5f, 16, GRAY); DrawCylinderWiresEx((Vector3){handPos.x, handPos.y, handPos.z}, (Vector3){handPos.x+0.1f, handPos.y, handPos.z}, 1.5f, 1.5f, 16, WHITE); }
        }
        if (tempoLaser > 0) { 
            DrawCylinderEx((Vector3){renderPos.x, renderPos.y + 1.0f, renderPos.z}, alvoLaser, 0.2f * escalaAtual, 0.2f * escalaAtual, 8, Fade(RED, 0.8f)); 
            DrawSphere(alvoLaser, 1.5f * escalaAtual, Fade(ORANGE, 0.9f)); 
        }
    }
};
...
void DrawCreditsMenu(int larguraTela, int alturaTela, Font font, GameState& estadoAtual, float dt) {
    ...
    DrawTextCenter("V25.0 - THE FOX SUPREMACY", startY + 60, 20, LIGHTGRAY);
...
// ==============================================================================
int main(void) {
    InitWindow(1920, 1080, "SUPERGEMINI CREATOR ENGINE - V25.0 THE FOX SUPREMACY");
    ToggleFullscreen(); 
    ...
    // INICIALIZA O JOGADOR E OS ANIMAIS (ZUBA DELETADO)
    PlayerCharacter jogador;
    jogador.Init();
    jogador.Carregar(globalShaderLuz);

    Animals.Init(globalShaderLuz);
    ...
    while (!WindowShouldClose() && !sairDoJogo) {
        ...
        if (estadoAtual == MENU) {
            ...
            DrawTextCenter("V25.0 THE FOX SUPREMACY", alturaTela/2 - 180, 20, LIGHTGRAY);
            ...
            for(int i = 0; i < 5; i++) {
                ...
                if (hover && IsKeyPressed(KEY_ENTER) && !IsKeyDown(KEY_LEFT_ALT)) {
                    Audio.PlayUISelect();
                    if (i == 0) { estadoAtual = PLAYING; if(!jogoIniciado) jogador.Init(); jogoIniciado = true; DisableCursor(); }
                    else if (i == 1) { 
                        ...
                        mundoMutex.unlock(); jogador.Init(); chatMinimizado = false; jogoIniciado = true; DisableCursor(); 
                    }
            ...
            if (!modoChat && !modoDiario) {
                ...
                if (IsKeyPressed(KEY_ONE)) { jogador.formaAtual = "FOX"; EmitirParticulas(jogador.posicao, WHITE, 20, 10.0f); Audio.PlayUISelect(); }
                if (IsKeyPressed(KEY_TWO)) { jogador.formaAtual = "ESFERA"; EmitirParticulas(jogador.posicao, ORANGE, 20, 10.0f); Audio.PlayUISelect(); }
                if (IsKeyPressed(KEY_THREE)) { jogador.formaAtual = "CUBO"; EmitirParticulas(jogador.posicao, GOLD, 20, 10.0f); Audio.PlayUISelect(); }
            }
            ...
            if (IsKeyPressed(KEY_T) && !modoChat && !modoDiario) { 
                ...
                if (IsKeyPressed(KEY_ENTER) && !inputAtual.empty()) {
                    ...
                    // A PROPRIA FOX VAI PROCESSAR NLP E EXECUTAR AS MÁGICAS NO MUNDO AGORA!
                    jogador.ProcessarConversa(inputAtual, aL, fwdDir); 
            ...
            if (modoChat) {
                ...
                Vector3 flatForward = Vector3Normalize((Vector3){forward.x, 0.0f, forward.z}); Vector3 right = { flatForward.z, 0.0f, -flatForward.x };
                ...
                if (godModeAtivo) {
                    ...
                    if (IsKeyDown(KEY_X)) moveDir.y -= 1.0f; 
                }
                ...
                jogador.Atualizar(rawDt, (Vector3){moveDir.x, 0, moveDir.z}, currentSpeed / velBase, !noChao && !godModeAtivo);

                if (isThirdPerson) {
                    float camDist = 7.0f;
                    camera.position = Vector3Subtract(jogador.posicao, Vector3Scale(forward, camDist));
                    camera.position.y += 3.0f; 
                    camera.target = (Vector3){jogador.posicao.x, jogador.posicao.y + 1.5f, jogador.posicao.z};
                } else {
                    float headOffset = noChao ? sin(headBobTimer) * 0.05f : 0.0f;
                    camera.position = (Vector3){jogador.posicao.x, jogador.posicao.y + 1.5f + headOffset, jogador.posicao.z};
                    camera.target = Vector3Add(camera.position, forward);
                }
            }

            Animals.Update(rawDt, jogador.posicao);
            ...
            if (cameraShake > 0) { cameraShake -= rawDt; cameraShakeOffset = (Vector3){ (RandFloat()-0.5f)*cameraShake, (RandFloat()-0.5f)*cameraShake, (RandFloat()-0.5f)*cameraShake }; }
            ...
            BeginDrawing();
                ...
                BeginMode3D(renderCam);
                    int rDistFinal = (climaGlobal == 3) ? 20 : RENDER_DIST; Vector3 lightDir = Vector3Normalize((Vector3){ -0.6f, 1.0f, -0.6f }); 
                    ...
                    auto GetVertexColor = [&](float vx, float vz, float vh) -> Color {
                        ...
                        Vector3 n = Vector3Normalize((Vector3){ hL - hR, 2.0f, hD - hU });
                        ...
                        for (int x = -rDistFinal; x < rDistFinal; x+=4) {
                            for (int z = -rDistFinal; z < rDistFinal; z+=4) {
                                ...
                                if (FBM(wX * 0.01f + GetTime() * 0.1f, wZ * 0.01f, 3) > 0.6f) { 
                                    Color cF = ApplyFog((Color){255, 255, 255, 200}, distCentro); rlColor4ub(cF.r, cF.g, cF.b, cF.a);
                    ...
                    if (climaGlobal == 4) { 
                        for (int i=0; i<100; i++) DrawCube((Vector3){camera.position.x + (float)(rand()%100-50), 60.0f + (float)sin(GetTime()+i)*10.0f, camera.position.z + (float)(rand()%100-50)}, 2.0f, 20.0f, 2.0f, Fade(GREEN, 0.3f));
                    }

                    if (climaGlobal == 0 && biomaAtual != 2 && biomaAtual != 3 && !modoWireframe && climaGlobal != 5) DrawSphere(Vector3Add(camera.position, Vector3Scale((Vector3){ -0.6f, 0.6f, -0.6f }, rDistFinal * 1.2f)), 20.0f, (Color){255, 250, 150, 255}); 
                    if (climaGlobal == 5) DrawSphere(Vector3Add(camera.position, Vector3Scale((Vector3){ -0.6f, 0.6f, -0.6f }, rDistFinal * 1.2f)), 20.0f, BLACK); 
                    ...
                    for (int x = -rDistFinal; x < rDistFinal; x++) {
                        for (int z = -rDistFinal; z < rDistFinal; z++) {
                            ...
                            if (modoWireframe) {
                                ...
                                rlEnd(); DrawLine3D((Vector3){wX, h00, wZ}, (Vector3){wX+1, h10, wZ}, cFinal); DrawLine3D((Vector3){wX+1, h10, wZ}, (Vector3){wX+1, h11, wZ+1}, cFinal); DrawLine3D((Vector3){wX+1, h11, wZ+1}, (Vector3){wX, h01, wZ+1}, cFinal); DrawLine3D((Vector3){wX, h01, wZ+1}, (Vector3){wX, h00, wZ}, cFinal); rlBegin(RL_QUADS);
                            } else {
                    ...
                    if (!modoWireframe && biomaAtual != 4) {
                        ...
                        for (int wX = startX; wX <= endX; wX+=2) {
                            for (int wZ = startZ; wZ <= endZ; wZ+=2) {
                                ...
                                Vector3 n = Vector3Normalize(Vector3CrossProduct(Vector3Subtract((Vector3){(float)wX, LerAlturaMundo(wX, wZ+1), (float)wZ+1}, (Vector3){(float)wX, h, (float)wZ}), Vector3Subtract((Vector3){(float)wX+1, LerAlturaMundo(wX+1, wZ), (float)wZ+1}, (Vector3){(float)wX, h, (float)wZ}))); if (n.y < 0) n = Vector3Scale(n, -1.0f); 
                                ...
                                if (biomaAtual == 0 && biomaAlvo == 0 && slope < 0.35f && h >= 0.0f && h < 18.0f && pathNoise > 0.04f) {
                                    ...
                                    if (prob > treeT && !Forest.trees.empty()) { int idx = (int)(prob * 10000) % Forest.trees.size(); DrawModelEx(Forest.trees[idx], (Vector3){(float)wX, h, (float)wZ}, (Vector3){0,1,0}, prob*360.0f, (Vector3){1.0f, 1.0f, 1.0f}, tint); } 
                                    else if (prob > bushT && !Forest.bushes.empty()) { int idx = (int)(prob * 10000) % Forest.bushes.size(); DrawModelEx(Forest.bushes[idx], (Vector3){(float)wX, h, (float)wZ}, (Vector3){0,1,0}, prob*360.0f, (Vector3){1.0f, 1.0f, 1.0f}, tint); } 
                                    else if (prob > rockT && !Forest.rocks.empty()) { int idx = (int)(prob * 10000) % Forest.rocks.size(); DrawModelEx(Forest.rocks[idx], (Vector3){(float)wX, h, (float)wZ}, (Vector3){0,1,0}, prob*360.0f, (Vector3){1.0f, 1.0f, 1.0f}, tint); } 
                                    else if (prob > grassT && !Forest.grasses.empty()) { int idx = (int)(prob * 10000) % Forest.grasses.size(); DrawModelEx(Forest.grasses[idx], (Vector3){(float)wX, h, (float)wZ}, (Vector3){0,1,0}, prob*360.0f, (Vector3){1.0f, 1.0f, 1.0f}, tint); }
                                }
                            }
                        }
                    }
                    ...
                    Animals.Draw(camera.position, (float)rDistFinal);
                    ...
                    for (const auto& ent : entidadesGlobais) {
                        if (ent.tipo == 1) { DrawSphere(ent.pos, ent.raio, BLACK); DrawSphereWires(ent.pos, ent.raio+0.5f, 16, 16, PURPLE); DrawCircle3D(ent.pos, ent.raio+2.0f, (Vector3){1,0,0}, 90.0f, Fade(PURPLE, 0.5f)); }
                        ...
                        else if (ent.tipo == 6) { DrawCube((Vector3){ent.pos.x, ent.pos.y+1, ent.pos.z}, 1.5, 2, 1.5, DARKGRAY); DrawCube((Vector3){ent.pos.x, ent.pos.y+3, ent.pos.z}, 1, 1, 1, GRAY); } 
                        else if (ent.tipo == 7) DrawCylinderWires(ent.pos, 2.0f, 2.0f, 0.1f, 16, MAGENTA); 
                        else if (ent.tipo == 8) { for(int d=0; d<5; d++) DrawSphere((Vector3){ent.pos.x - d*1.5f, ent.pos.y + (float)sin(GetTime()*5+d)*0.5f, ent.pos.z}, 1.0f - d*0.1f, RED); DrawCube((Vector3){ent.pos.x+1, ent.pos.y+0.5f, ent.pos.z+0.5f}, 0.5f, 0.5f, 0.5f, YELLOW); DrawCube((Vector3){ent.pos.x+1, ent.pos.y+0.5f, ent.pos.z-0.5f}, 0.5f, 0.5f, 0.5f, YELLOW); } 
                        else if (ent.tipo == 9) { DrawCylinder(ent.pos, 5.0f, 5.0f, 1.0f, 16, GRAY); DrawSphere((Vector3){ent.pos.x, ent.pos.y+0.5f, ent.pos.z}, 2.0f, Fade(SKYBLUE, 0.7f)); DrawCylinderEx(ent.pos, (Vector3){ent.pos.x, ent.pos.y-10.0f, ent.pos.z}, 4.0f, 8.0f, 16, Fade(GREEN, 0.2f)); } 
                        else if (ent.tipo == 10) { DrawCylinderEx((Vector3){ent.pos.x, ent.pos.y+100.0f, ent.pos.z}, ent.pos, ent.raio, ent.raio, 16, Fade(SKYBLUE, 0.8f)); DrawSphere(ent.pos, ent.raio*1.5f, WHITE); } 
                    ...
                    if (climaGlobal == 1 || climaGlobal == 2) {
                        for (int i=0; i<300; i++) { Vector3 pR = (Vector3){camera.position.x + (rand()%40-20), camera.position.y + (rand()%20), camera.position.z + (rand()%40-20)}; DrawLine3D(pR, (Vector3){pR.x, pR.y-1.0f, pR.z}, Fade(BLUE, 0.5f)); }
                    }
                    if (escudoAtivo) DrawSphereWires(jogador.posicao, 3.0f, 16, 16, Fade(SKYBLUE, 0.5f));

                    // DESENHAR JOGADOR
                    if (isThirdPerson) {
                        jogador.Desenhar(godModeAtivo, globalShaderLuz, camera.position);
                    }
                ...
                if (!modoDiario) {
                    std::lock_guard<std::mutex> lock(jogador.chatMutex);
                    ...
                    if (!chatMinimizado) {
                        ...
                        for (const auto& msg : jogador.historico) {
                        ...
                        if (jogador.forcadoScroll) { chatScrollIndex = maxScroll; jogador.forcadoScroll = false; }
                    ...
                    } else DrawTextEx(fUniv, jogador.pensando ? Config.Tr("Sistema processando...", "System processing...") : Config.Tr("Sistema aguarda. (Chat Oculto)", "System awaits. (Hidden Chat)"), {30.0f, (float)startY}, 20, 1, GRAY);

                    if (jogador.pensando && !chatMinimizado) DrawTextEx(fUniv, Config.Tr("Processando Magia NLP...", "Processing NLP Magic..."), {30.0f, (float)alturaTela - 60}, 16, 1, GRAY);
